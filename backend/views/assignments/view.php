...
<table class="table striped hovered dataTable" width="100%">
	...
	<tr>
        ...
        <th width="12%" class="text-left">Submission Date</th>
        <th class="text-left">Submitted ?</th>  
        <th width="12%" class="text-left">Marks</th>
        <th class="text-left">Comments</th>         
    </tr>
    </thead>
    <?php
	 
	$submittedAssignments = Submission::find()->where("AssignmentID = '".$model['AssignmentID']."'")->asArray()->all();      
	// print_r($submittedAssignments)    ;
	//exit;         
    foreach ($submittedAssignments AS $key => $row)
    {    
		extract($row);
		$CreatedDate 	= date('d/m/Y',strtotime($CreatedDate));
		?>
    <tr>
    	<td class="text-left"><?php echo $CreatedDate; ?></td>
    	<td class="text-left"><?php echo $ProfileID; ?></td>
    	<td class="text-left"><?php echo $SubmissionDate; ?></td>
    	<?php
    	if ($Submitted ==1) {
    		?>
