...
?>
<script>
	var serverurl = 'http://localhost:8080/ritman/backend/web';
	...
	function loadsemester()
	{
		...
		if (Programme != '') 
		{
			var url = '/examregistration/getsemester?Programme='+Programme;
	...
	function loadstream()
	{
		...
		{
			var url = '/examregistration/getstream?Programme='+Programme+'&AcademicYear='+AcademicYear+'&Semester='+Semester+'&Stage='+Stage;
			fetch_data(url,function(response) 
			{
    ...
    function loadcourse()
	{
		...
		if ((Programme != '') && (AcademicYear != '') && (Semester != '') && (Stage != ''))
		{
			var url = '/examregistration/getcourse?Programme='+Programme+'&AcademicYear='+AcademicYear+'&Semester='+Semester+'&Stage='+Stage;
			fetch_data(url,function(response) 
			{
...
<form id="data_form" method="POST" action="<?= $url; ?>">
	...
	<tr>
		...
		<td width="50%">
			<label>Assignment <span style="color:#F00">*</span></label>
			<div data-role="select"> 
				<select name="ExamTypeID"  id="ExamTypeID" class="input-control full-size" onchange="loadcourse()">
					<option value="0" selected="selected"></option>
					<?php 
					$result = Examtypes::find()->asArray()->orderBy('Description')->where(['Assignment' => 1])->all();                    
					foreach ($result AS $key => $row)
					{
						$s_id = $row["Code"];
						$s_name = $row["Description"];
						if ($model['Exam Type ID']==$s_id) 
						{
							$selected = 'selected="selected"';
						} else
						{
