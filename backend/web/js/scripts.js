...
function onQuantityInputBlur(inp) {	
	if (inp.value) 
	{
		QuantityInputs[inp.name] = parseInt(inp.value)
		onUnitPriceInputBlur(inp)
	}
		
}

function onUnitPriceInputBlur(inp) {
	var input_key = inp.name.split("_").splice(0,2).join("_") + "_Quantity"	
	if (inp.value && QuantityInputs[input_key]) {
		//UnitPriceInputs[inp.name] = parseInt(inp.value)
		calculateAmount(inp.name.split("_")[1])
	}
	
}

function calculateAmount(line) {
	quantity_key = "lines_" + line + "_Quantity"
	unitprice_key = "lines_" + line + "_UnitPrice"
	amount = QuantityInputs[quantity_key] * UnitPriceInputs[unitprice_key]

	renderAmount(line, amount)
}

function renderAmount(line, amount) {
	el = "lines_" + line + "_Amount"
	el_name = "input[name=" + el + "]"
		var value = parseFloat(amount);
	$(el_name).val(value.toFixed(2))
}


function filterOptions(arg) {
	//console.log(arg.value)

	switch(parseInt(arg.value)) {
		case 1: 
			filterItems(arg)
			break
		case 2: 
			...
			break
		case 3: 
			console.log(arg.value, 'to be done later')
			break
	}
}
...
function filterItems(arg) {
	var line = arg.name.split("_")[1]
	var target_el = 'select[name="lines_' + line + '_No"]'
	var items_options = items.map(function(item) {
		return ('<option value="' + item.No_ + '"> ' + item.Description + ' </option>')
	}).join(' ')
	$(target_el).empty().append(items_options)
}

function filterAccounts(arg) {
	var line = arg.name.split("_")[1]
	var target_el = 'select[name="lines_' + line + '_No"]'
	var accounts_options = accounts.map(function(account) {
		return ('<option value="' + account.No_ + '"> ' + account.No_ + " : " + account.Name + ' </option>')
	}).join(' ')
	$(target_el).empty().append(accounts_options)
}

function autofillDescription(arg) {
	var line = arg.name.split("_")[1]
	var src_el = 'select[name="lines_' + line + '_No"]' + '  option[value=' + arg.value + ']'
	var desc_el = 'input[name="lines_' + line + '_Descsription"]'
	var unitprice_el = 'lines_' + line + '_UnitPrice'
	console.log(unitprice_el);

	$(desc_el).val( $(src_el).text() )
	$.ajax({url: "getunitprice", 
		type:'GET',
		...
		success: function(result){
			...
			UnitPriceInputs[unitprice_el] = parseInt(value.toFixed(2))
        $('input[name="'+unitprice_el+'"]').val(value.toFixed(2));
    }});
